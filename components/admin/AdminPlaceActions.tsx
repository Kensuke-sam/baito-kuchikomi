...
export function AdminPlaceActions({ id }: { id: string }) {
  ...
  async function update(status: "approved" | "rejected") {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readErrorMessage(res, "勤務先の更新に失敗しました。"));
      }

      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "勤務先の更新に失敗しました。");
    } finally {
      setLoading(null);
    }
  }
